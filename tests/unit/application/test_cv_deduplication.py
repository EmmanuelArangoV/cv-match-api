...
async def test_cv_deduplication():
    ...
    mock_process = HiringProcess(
        id=uuid.uuid4(),
        status=ProcessStatus.CVS_UPLOADED.value,
        budget_max_usd=100.0,
    )

    with (
        patch("src.application.cv.use_cases.ProcessRepository") as mock_process_repo,
        ...
        with patch("src.application.cv.use_cases.hashlib.sha256") as mock_hash:
            ...
            results = await use_case.execute(
                process_id=mock_process.id, files=[mock_file], uploader_id=uuid.uuid4()
            )
